    ...
    <form class="mt-6 space-y-4" method="POST" action="{{ $isEdit ? route('sku_serial_formats.update', $format) : route('sku_serial_formats.store') }}">
        ...
        <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <ol class="mt-2 space-y-1 text-sm text-slate-700 list-decimal list-inside">
                ...
                <li><strong>Paso 2:</strong> Configura prefijo de producto, versión y QR.</li>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="anz_serial_print_format" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2">
                    <option value="spaces" @selected(old('anz_serial_print_format', $format->anz_serial_print_format ?? 'spaces') === 'spaces')>Con espacios</option>
                    <option value="no_spaces" @selected(old('anz_serial_print_format', $format->anz_serial_print_format ?? null) === 'no_spaces')>Sin espacios</option>
                    <option value="segmented" @selected(old('anz_serial_print_format', $format->anz_serial_print_format ?? null) === 'segmented')>Segmentado</option>
                </select>
            ...
            <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-3">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-500">Paso 2 · Estructura ANZ · PPPPPPPPP A XXXXX MJJJJ</p>
                    <p class="mt-1 text-sm text-slate-600">Puedes activar/desactivar código de herramienta de cliente dentro del QR.</p>
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" min="1" max="10" name="anz_unit_digits" value="{{ old('anz_unit_digits', $format->anz_unit_digits ?? 5) }}" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" />
