    ...
    <form class="space-y-6" method="POST" action="{{ route('admin.dummy_qr_templates.update', $template) }}">
        ...
        <div class="sticky bottom-4 z-10 rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-lg backdrop-blur">
            <button
                ...
                data-loading-label="Actualizando template..."
                class="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 py-3 font-semibold text-white hover:bg-slate-800 disabled:cursor-wait disabled:opacity-70"
            >
                ...
                <span data-dummy-template-submit-label>Actualizar template Dummy QR</span>
            </button>
        </div>
    </form>
</div>
