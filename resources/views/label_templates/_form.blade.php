...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div>
        ...
        <select name="label_sku_id" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2">
            <option value="">Global</option>
            @foreach($labelSkus as $sku)
                <option value="{{ $sku->id }}" @selected((string) old('label_sku_id', $template->label_sku_id ?? '') === (string) $sku->id)>{{ $sku->sku }} · {{ $sku->label_part_number }}</option>
            @endforeach
        </select>
    </div>
    ...
    <div class="md:col-span-2"><label class="block text-sm font-medium text-slate-700">ZPL</label><textarea name="zpl" rows="10" class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2" required>{{ old('zpl', $template->zpl ?? '') }}</textarea></div>
