...
class UpdateSkuSerialFormatRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            ...
            'serial_scheme' => trim((string) $this->input('serial_scheme', $standard === SerialStandards::UL ? 'ul_standard' : ($standard === SerialStandards::EMEA ? 'emea_rating' : 'anz_standard'))),
            'description' => $this->input('description'),
            'serial_length' => $this->input('serial_length'),
            'qr_payload_format' => trim((string) $this->input('qr_payload_format', $standard === SerialStandards::ANZ ? 'customer_tool_code_serial' : ($standard === SerialStandards::EMEA ? 'emea_code_only' : 'serial_only'))),
            'date_mode' => trim((string) $this->input('date_mode', $standard === SerialStandards::UL ? 'year_week' : 'month_year')),
